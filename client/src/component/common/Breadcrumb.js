...
const Breadcrumb = ({title, category, brand, pages}) => {
  ...
  if (brand) {
    ...
    // { path : '/', breadcrumbs : "Home"},
      ...
      {path : '/products/:category/:brand', breadcrumbs : brand},
   ...
   if (category) {
    ...
    if (pages) {
      routes = [
        { path : '/pages/:page', breadcrumbs : pages},
      ]
    }
  }
      ...
      <div className='w-main flex flex-col gap-2'>
        ...
        <div className='flex items-center gap-1'>
          <Link to={breadcrumbs[0].match.pathname} className='flex items-center gap-1 text-sm'>
            ...
            <FaChevronRight size={10} className='mt-1 text-[#505050] mb-1'/>
          </Link>
          {breadcrumbs?.filter(el => el.match.route).map(({ match, breadcrumb }, index, self) => (
            <Link key={match.pathname} to={match.pathname} className='flex items-center gap-1 text-sm'>
